    ...
    async def populate_qdrant(self, force_repopulate=False):
        ...
        if not force_repopulate:
            ...
            if collection_info and collection_info.get("vectors_count", 0) > 0:
                print(f"Collection '{self.qdrant_manager.qdrant_collection}' already contains {collection_info['vectors_count']} vectors")
                print(" Use force_repopulate=True to overwrite existing data")
                return True
        ...
        if force_repopulate:
            try:
                ...
                self.qdrant_manager.embedding_vector_size = int(os.getenv("EMBEDDING_VECTOR_SIZE", "4096"))
        ...
        try:
            ...
            collection_info = self.qdrant_manager.get_collection_info()
            if collection_info:
                print(f" Collection info reports: {collection_info['vectors_count']} vectors")
            ...
            if vectors_verified and len(test_results) > 0:
                ...
                print("  Vector storage verification failed - vectors may not be properly stored")
